...
// TestNilGuardsAreDirectlyExercised pins the nil guards this package's Value
// methods carry. Each guard's comment says it exists for a DIRECT call, where
// nothing above recovers — and since agent.IdentityFromContext gained its own
// recover, reaching them through that entry point proves nothing. These calls put
// nothing between the shape and the guard.
func TestNilGuardsAreDirectlyExercised(t *testing.T) {
	type probeKey struct{}
	for _, tc := range []struct {
		name string
		ctx  context.Context
	}{
		{"typed-nil invocation context", (*icontext.InvocationContext)(nil)},
		{"typed-nil readonly context", (*icontext.ReadonlyContext)(nil)},
		{"readonly context with no invocation", &icontext.ReadonlyContext{}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			defer func() {
				if p := recover(); p != nil {
			...
			// The identity key too: it is the one key that leaves the delegation
			// path, so a guard on that branch is not reached by the probe above.
			if v := tc.ctx.Value(adkcontext.IdentityKey); v != nil {
				t.Errorf("Value(IdentityKey) = %v, want nil", v)
			}
		})
	}
}

// Note on one guard this does NOT pin. ReadonlyContext.Value's inner
// `c.InvocationContext == nil` check is redundant: deleting it keeps every test
// here green, because agent.Promote(nil) yields a commonContext whose own nil
// check answers the same way. The contract is pinned above; the guard is
// belt-and-braces and saves an allocation, and saying so is better than counting
// it as covered.
