...
func NewToolContext(ctx agent.InvocationContext, functionCallID string, actions *session.EventActions, confirmation *toolconfirmation.ToolConfirmation) tool.Context {
	...
	// agent.NewCallbackContextWithDelta already wraps Artifacts with
	// delta tracking against the supplied EventActions, so toolContext
	// just inherits Artifacts() from the embedded CallbackContext.
