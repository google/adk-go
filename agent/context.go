...
// The dynamic-node scheduler is intentionally NOT part of the public
// Context interface: it is a workflow-only concern (the scheduler type
// lives in package workflow). The agent layer only carries it as an
// opaque token, set via NewNodeContext and read back by package
// workflow through the concrete commonContext.NodeScheduler() accessor.
// This keeps Context's public surface free of scheduling concepts,
// mirroring adk-python where _workflow_scheduler is a private field.
