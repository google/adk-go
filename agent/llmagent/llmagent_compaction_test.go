...
//
// The directive below claims exactly this one cassette, and nothing else in the
...
// scoped to the FunctionTool, LLMAgent and ToolCallback cassettes -- so this one
// was left claimed by nothing and could not be re-recorded by go generate.
//
// Note that a failed recording still leaves a cassette behind, and it can look
// plausibly sized because the failing exchange is recorded too. Delete it, or
// the next run replays the recorded failure.
//
