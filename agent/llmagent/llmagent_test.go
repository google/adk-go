...
func TestToolCallback(t *testing.T) {
	...
	handler := func(_ context.Context, _ agent.Context, input Args) (Result, error) {
		...
		},
		{
			...
			llmagentFunc: func(model model.LLM) (agent.Agent, error) {
				return llmagent.New(llmagent.Config{
					...
					InstructionProvider: func(_ context.Context, ctx agent.ReadonlyContext) (string, error) {
		...
		},
		{
			...
			llmagentFunc: func(model model.LLM) (agent.Agent, error) {
				return llmagent.New(llmagent.Config{
					...
					GlobalInstructionProvider: func(_ context.Context, ctx agent.ReadonlyContext) (string, error) {
...
func TestLLMAgent_WorkflowIntegration_OutputPropagatesToSuccessor(t *testing.T) {
	...
	var gotInput any
	fnNode := workflow.NewFunctionNode("receiver", func(_ context.Context, ctx agent.Context, in any) (any, error) {
		gotInput = in
		return nil, nil
	}, workflow.NodeConfig{})
	...
	exCtx := agent.NewNodeContext(runCtx, mockCtx, nil)

	events := w.Run(runCtx, exCtx)
