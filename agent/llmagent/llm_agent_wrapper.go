...
//
//   - single_turn: current-turn contents only (derived from Mode in the
//     contents processor), seeds the agent with a single user-content
//     event derived from nodeInput, drives one Agent.Run, post-processes
//     the model reply into the terminal Output, then returns.
...
func RunLLMAgentAsNode(a agent.Agent, ctx agent.Context, nodeInput any) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		// Resolve ModeUnset once without racing parallel node dispatches.
		// single_turn IncludeContents=none is derived at read time in the
		// contents processor (do not mutate shared State here).
		mode := state.ResolveMode(llminternal.ModeSingleTurn)
		switch mode {
