...
// races it.
//
// Concurrency-safe is necessary and not sufficient, and the gap is the one an
// implementer falls into. The read binds one [session.Session] and then makes
// three separate accessor calls on it, so a session that guards each accessor
// with a mutex has met "safe for concurrent reads" exactly and can still hand
// back an Identity whose three fields name two different generations — a user
// against another's app and session. What is required is a consistent triple
// across three consecutive calls on one Session value. Fixing all three at
// construction gives you that, and is what ADK's own sessions do.
//
// Build one with keyed fields. Fields may be added, and an unkeyed literal stops
// compiling when they are — Go's compatibility promise permits the addition and
...
//
// "Package agent" is doing work in that sentence. internal/context holds a
// second copy, because it cannot import this package without a cycle. The two
// must agree field for field; TestIdentityMappingsAgree in that package is what
// holds them together, and it fails if a field added to Identity reaches only
// one of them.
//
...
//
// Read this before acting on the result: it establishes which invocation a
// context was DERIVED from, and is trusted only as far as every wrapper between
// it and that invocation is. It is NOT an authentication boundary and cannot be
// made into one. ok does not rule out the answer belonging to an ENCLOSING
// invocation: an [InvocationContext] implemented outside the module, passed as
// the context itself, reports the invocation it embeds rather than its own
// session. The rule below says exactly when, and why closing it is not possible
// here.
//
// It returns (zero, false) and never panics. Several different things produce
// that answer and none is distinguishable from the others here — a context that
// does not descend from an ADK context, an invocation with no readable session, a
// nil ctx, an intermediary in the chain whose Value panics, a context answering
// the key with something that is not an [Identity]. The list is not closed, and
// callers must not treat "false" as naming any one of them. A panic is contained
// rather than propagated, because this runs where net/http does not recover. ok does not imply a populated Identity either: an
// invocation whose session carries no user yields an empty UserID, so a caller
// that needs one must check.
//
// An invocation ADK itself built reports its own user however a context is
// derived from it, with one exception that applies in-module too: [Context.WithContext]
// and [Context.WithAgentContext] rebind to the argument when handed an
// [InvocationContext] rather than a plain context.Context, so the result reports
// the ARGUMENT's user by design. That case is set out below. An
// [InvocationContext] implemented OUTSIDE the module is where the rest stops, and
// one rule covers every way it does.
//
...
//     unnameable outside the google.golang.org/adk/v2 import-path tree — Go scopes
//     internal/ by import path and not by module, so a separate module published
//     under that prefix is inside the boundary — but this function hands the key VALUE to
...
func IdentityFromContext(ctx context.Context) (Identity, bool) {
	// Recovered for the same reason every inner read is: this runs on the
	// caller's goroutine inside http.RoundTripper, where net/http does not
	// recover, and ctx here is whatever the transport was handed. Any
	// intermediary between it and the ADK context supplies this Value, so an
	// intermediary that panics would take the process down rather than cost the
	// identity — and the inner recovers cannot help, because the panic happens
	// before the chain reaches one. It also covers a nil ctx.
	v, _ := adkcontext.Recovered(func() any { return ctx.Value(adkcontext.IdentityKey) })
	id, ok := v.(Identity)
	return id, ok
}
