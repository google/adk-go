...
func runBeforeAgentCallbacks(ctx InvocationContext) (*session.Event, error) {
	...
	callbackCtx := NewCallbackContextWithDelta(ctx, stateDelta, artifactDelta)
	actions := func() session.EventActions {
		return session.EventActions{StateDelta: stateDelta, ArtifactDelta: artifactDelta}
	}
