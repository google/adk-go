...
// ErrInvalidResumeResponse is returned during resume when a response
// payload does not match its interrupt's response schema (re-extracted
// from the pause event during rehydration). The node stays paused so
// the caller can retry with a corrected payload; a later valid payload
// for the same interrupt supersedes the rejected one (last-wins).
...
// RunState rehydrated from session history (ReconstructRunState);
// responses maps an interrupt's InterruptID to the user-supplied
// response payload.
//
// For each rehydrated node, Resume acts only on interrupts answered
// in THIS turn (present in responses). Gating on the current turn's
// responses keeps Resume idempotent: a duplicate turn whose responses
// target an already-consumed interrupt reschedules nothing. Depending
// on the node:
//
//  1. Re-entry (RerunOnResume) or partial resume: the node re-runs
//     with the resolved responses delivered via ctx.ResumedInput;
//     successors fire only when the re-entry produces an output.
//
//  2. Handoff: the response becomes the asker's output, forwarded to
//     its successors; the asker itself does NOT re-execute. A
//     successor already completed on a prior turn (state.completed) is
//     skipped so it does not re-run.
//
// Fresh responses are validated against the interrupt's response
// schema; a mismatch surfaces as ErrInvalidResumeResponse via the
// iterator and leaves the node paused. (Schemas attached during
// rehydration are validated in inferNodeState.)
//
// If responses is non-empty but nothing was scheduled, Resume yields
// ErrNothingToResume so the caller can distinguish a successful resume
// from a stale or mistargeted submission. An empty responses map (or
// nil state) is treated as a clean no-op with no error.
