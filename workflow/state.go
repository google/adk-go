...
//
// NodeState is not persisted as a blob; it is rebuilt each turn by
// ReconstructRunState scanning session history (see persistence.go).
// The Input and Output fields are typed any; nodes that need to
// carry binary data across a pause should store the bytes via
// agent.Artifacts and stash a URI string in place of the bytes so
// the value survives history rehydration.
...
// run. It is not persisted; ReconstructRunState rebuilds it each
// turn from session history so the workflow can pick up where a
