...
func TestValidateChatModeWiring(t *testing.T) {
	...
	t.Run("rejects chat-mode agent following a non-Start node", func(t *testing.T) {
		t.Parallel()
		responder := newWiringNode(t, "responder", llmagent.ModeSingleTurn)
		coordinator := newWiringNode(t, "coordinator", llmagent.ModeChat)
	...
	t.Run("allows single_turn agent following a non-Start node", func(t *testing.T) {
		t.Parallel()
		first := newWiringNode(t, "first", llmagent.ModeSingleTurn)
		second := newWiringNode(t, "second", llmagent.ModeSingleTurn)
...
func newWiringNode(t *testing.T, name string, mode llmagent.Mode) workflow.Node {
	t.Helper()
