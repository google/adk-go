...
//
// Long-running interrupts are the exception: a child's pause rides
// on Event.LongRunningToolIDs, which trackInterrupts accumulates
// before the descendant short-circuit so the pause is promoted to
// the parent node. Workflow.Resume later matches a human's
// FunctionResponse against the parent's NodeState.Interrupts, so the
// parent must transition to NodeWaiting on a descendant pause.
...
//
// # Long-running interrupt waiting branch
//
// When an activation completes cleanly with open long-running
// interrupts accumulated by trackInterrupts (a RequestInput pause
// rides on Event.LongRunningToolIDs), the node transitions to
// NodeWaiting instead of NodeCompleted, the open IDs are recorded on
// NodeState.Interrupts, and successors are not scheduled. The
// scheduler's main loop terminates naturally when every live node
// has either completed or moved into NodeWaiting, at which point
// Workflow.Run's iterator exhausts and the next turn rehydrates the
// pause from session history (see ReconstructRunState). A node may
// raise more than one interrupt per activation.
//
// The waiting branch is checked after the error/cancel branches,
// so a node that fails for any reason (returned error, panic,
// context cancel, multiple-output, multiple-routing-event) does not
// silently park in NodeWaiting: failures take precedence and surface
// as NodeFailed.
