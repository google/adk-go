...
//
// The name identifies the workflow; it is not used to persist a
// RunState blob. Paused state is rebuilt each turn from session
// history by ReconstructRunState, so there is no per-name state
// key to collide on and an empty name does not disable resume.
//
...
// Name returns the workflow's name as set by New. May be empty for
// an anonymous workflow. The name is identity only; resume state is
// rehydrated from session history, not keyed by name.
