...
func e2eScenarios() []scenario {
	return []scenario{
		...
		// --- where the rules have to be APPLIED, not recalled ------------------
		//
		// Every scenario above is the textbook instance of one prompt bullet:
		// a gofmt change for "formatting", a .pb.go regeneration for "generated
		// code". A capable model decides those from general knowledge, so they
		// cannot tell a prompt that states its rules from one that does not --
		// which is why mutation testing found the rule sections inert, deleted
		// singly or together.
		//
		// These three put the two signals in CONFLICT. Each looks, by path and
		// commit subject, like a category the prompt says to ignore, and each
		// actually changes something a user can see. Getting them right needs
		// the rule applied rather than the label matched, so they are the
		// scenarios a rule deletion could plausibly move.
		{
			name: "files_an_internal_change_that_moves_a_documented_default",
			why:  "conflict: the path says internal refactor, the substance is a changed default",
			files: []*github.CommitFile{file("internal/config/defaults.go",
				"@@ -14,7 +14,7 @@\n"+
					" // DefaultRequestTimeout is the timeout applied when Config.Timeout is unset.\n"+
					" // It is the value documented in the configuration reference.\n"+
					"-const DefaultRequestTimeout = 30 * time.Second\n"+
					"+const DefaultRequestTimeout = 5 * time.Second\n")},
			cmts: []*github.RepositoryCommit{commit("d15c0001", "refactor(internal): tidy the defaults table")},
			want: want{filed: true, kinds: []string{"behavior-change", "breaking-change", "config-change"}},
		},
		{
			name: "files_a_dependency_bump_that_changes_observable_behavior",
			why:  "conflict: the commit says dependency bump, the substance is a changed default",
			files: []*github.CommitFile{
				file("go.mod", "@@ -8,3 +8,3 @@\n-\tgolang.org/x/net v0.30.0\n+\tgolang.org/x/net v0.31.0\n"),
				file("transport/http.go",
					"@@ -22,6 +22,9 @@\n"+
						"-// Redirects are followed automatically, up to ten hops.\n"+
						"-client := &http.Client{}\n"+
						"+// Redirects are NO LONGER followed: x/net v0.31 changed the default and\n"+
						"+// callers must opt in with Config.FollowRedirects.\n"+
						"+client := &http.Client{CheckRedirect: noRedirect}\n"),
			},
			cmts: []*github.RepositoryCommit{commit("d15c0002", "chore(deps): bump golang.org/x/net")},
			want: want{filed: true, kinds: []string{"behavior-change", "breaking-change", "config-change"}},
		},
		{
			name: "files_generated_code_that_adds_public_surface",
			why:  "conflict: the file is generated, the substance is a new exported field users set",
			files: []*github.CommitFile{file("proto/agent.pb.go",
				"@@ -1,4 +1,12 @@\n"+
					" // Code generated by protoc-gen-go. DO NOT EDIT.\n"+
					"+\n"+
					"+// RetryPolicy configures how the agent retries a failed tool call.\n"+
					"+// Callers set it on AgentConfig; leaving it nil disables retries.\n"+
					"+type RetryPolicy struct {\n"+
					"+\tMaxAttempts int32 `protobuf:\"varint,1,opt,name=max_attempts\"`\n"+
					"+\tBackoff     int64 `protobuf:\"varint,2,opt,name=backoff\"`\n"+
					"+}\n")},
			cmts: []*github.RepositoryCommit{commit("d15c0003", "chore: regenerate protos")},
			want: want{filed: true, kinds: []string{"new-feature", "config-change"}},
		},
