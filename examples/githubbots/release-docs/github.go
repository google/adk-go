...
func (c *GitHubClient) isOwnMarkedIssue(iss *github.Issue, marker string) bool {
	...
	// Author and first-line marker, deliberately NOT the title. Requiring the
	// title would narrow the App-authorship fallback, but a maintainer who
	// retitles the issue during triage -- an ordinary thing to do -- would then
	// make the next run file a duplicate, which is the guarantee that actually
	// matters. The residual is documented in README.md.
