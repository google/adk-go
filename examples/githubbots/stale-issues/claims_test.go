...
func staleReady() IssueState {
	return IssueState{
		...
		LastActionType:  string(eventCommented),
		LastCommentText: "Could you share a minimal repro?",
		IsStale:         false, DaysSinceActivity: 30, DaysSinceLastActorAction: 30,
		// The author last spoke before the maintainer asked, which is what
		// "waiting on the author" means.
		DaysSinceAuthorAction: 35,
...
func TestClaimIsPerActionNotPerIssue(t *testing.T) {
	...
	// The author came back AFTER the label went on, which is what STEP 1's
	// remove branch is for. removeStalePredicate refuses the reverse ordering.
...
// Defence in depth for the case validate() already rejects: if the collision
// check at load time were ever loosened, the tool must still refuse.
//
// Asserting only res.Status == "error" made this test unable to fail. With the
// refusal reverted to an exact match the call fell through to claimAction, which
// errors too because no observation was recorded, so the test passed on the
// broken implementation. Naming the gated tool in the assertion is what makes it
// discriminate. The ordinary, non-colliding case is
// TestAddLabelRefusesTheStaleLabelInAnyCasing.
func TestStaleLabelRefusalIsCaseInsensitive(t *testing.T) {
	...
	if !strings.Contains(res.Message, "add_stale_label_and_comment") {
		t.Errorf("refusal = %q, want it to name the gated tool; any other refusal means the stale check was skipped", res.Message)
	}
}
...
func TestAddClarificationRefusedBelowThreshold(t *testing.T) {
	...
	c.cfg.RequestClarificationLabel = "request clarification"
	st := staleReady()
	st.DaysSinceLastActorAction = 1 // well inside the threshold
	c.recordObservation(7, st)
