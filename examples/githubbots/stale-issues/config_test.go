...
func setRequiredCreds(t *testing.T) {
	...
	for _, k := range []string{
		...
		"CONCURRENCY_LIMIT", "ISSUE_TIMEOUT", "RUN_BUDGET", "DRY_RUN", "BOT_LOGIN",
	} {
		t.Setenv(k, "")
	}
}
...
func TestLoadConfig_Defaults(t *testing.T) {
	...
	if cfg.IssueTimeout != 5*time.Minute {
		t.Errorf("IssueTimeout = %v, want 5m", cfg.IssueTimeout)
	}
	if cfg.RunBudget != 30*time.Minute {
		t.Errorf("RunBudget = %v, want 30m", cfg.RunBudget)
	}
	if cfg.DryRun {
...
// RUN_BUDGET is set by the workflow, so the env path has to work: a budget that
// silently fell back to the default would sit above the job's timeout-minutes
// and the overrun would be a SIGKILL again.
func TestLoadConfigRunBudgetFromEnv(t *testing.T) {
	setRequiredCreds(t)
	...
	if cfg.RunBudget != 90*time.Second {
		t.Errorf("RunBudget = %v, want 90s from RUN_BUDGET", cfg.RunBudget)
	}
}
