...
var (
	...
	// These must DIFFER. With both at 168h, swapping the two assignments in
	// computeIssueState is invisible to every test that reads them.
	testStaleAfter = 336 * time.Hour // 14 days
...
func TestComputeIssueStateThresholdsReported(t *testing.T) {
	...
	if got.StaleThresholdDays != 14 {
		t.Errorf("StaleThresholdDays = %v, want 14 (from staleAfter)", got.StaleThresholdDays)
	}
	if got.CloseThresholdDays != 7 {
		t.Errorf("CloseThresholdDays = %v, want 7 (from closeAfter)", got.CloseThresholdDays)
	}
