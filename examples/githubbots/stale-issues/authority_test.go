...
// has answered: the shape every mark-stale precondition is written against.
...
func TestMarkStalePreconditionEnforcedInCode(t *testing.T) {
	cases := []struct {
		...
		{"threshold not met", func() *IssueState {
			s := okState()
			s.DaysSinceLastActorAction = 14 // equal to the threshold, not past it
			return &s
		}(), "stale threshold"},
		// A rename, a reopen or a label change all make a maintainer the last
		// actor without anything having been asked, and the warning comment the
		// bot would post claims a request for clarification was made.
		{"maintainer acted but asked nothing", func() *IssueState {
			s := okState()
			s.LastActionType = string(eventReopened)
			s.LastCommentText = ""
			return &s
		}(), "no maintainer comment to judge"},
	}
...
func TestValidateRejectsNonPositiveDurations(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Without a positive budget the whole-run timeout is instantly expired,
		// so the sweep would abort before auditing anything.
		{"zero run budget", func(c *Config) { c.RunBudget = 0 }, "RUN_BUDGET"},
		{"negative run budget", func(c *Config) { c.RunBudget = -1 }, "RUN_BUDGET"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			c := base()
			tc.mut(c)
