...
// A thread longer than the fetch window leaves comments behind that the model
// is never shown. FetchIssue records how many, so assembleSuspectText can
// declare them: without the count the prompt is indistinguishable from a
// complete thread, and the model answers "no spam" over content it never saw.
func TestFetchIssueRecordsCommentsLeftBehindByTheWindow(t *testing.T) {
	const body = `{"data":{"repository":{"issue":{
		"number":5,"title":"t","body":"b",
		"author":{"login":"alice","__typename":"User"},
		"authorAssociation":"NONE","labels":{"nodes":[]},
		"comments":{"totalCount":137,"nodes":[
			{"author":{"login":"bob","__typename":"User"},"authorAssociation":"NONE","body":"hi"}
		]}}}}}`
	c := respondWith(t, testConfig(), body)
	iss, err := c.FetchIssue(context.Background(), 5)
	if err != nil {
	...
	if iss.UnfetchedComment != 136 {
		t.Errorf("UnfetchedComment = %d, want 136 (137 total minus the 1 fetched)", iss.UnfetchedComment)
	}
}

// A totalCount below the node count would otherwise produce a negative,
// nonsensical omission count in the prompt.
func TestFetchIssueClampsAnImpossibleCommentTotal(t *testing.T) {
	const body = `{"data":{"repository":{"issue":{
		"number":5,"title":"t","body":"b",
		"author":{"login":"alice","__typename":"User"},
		"authorAssociation":"NONE","labels":{"nodes":[]},
		"comments":{"totalCount":0,"nodes":[
			{"author":{"login":"bob","__typename":"User"},"authorAssociation":"NONE","body":"hi"},
			{"author":{"login":"carol","__typename":"User"},"authorAssociation":"NONE","body":"yo"}
		]}}}}}`
	c := respondWith(t, testConfig(), body)
	iss, err := c.FetchIssue(context.Background(), 5)
	if err != nil {
	...
	if iss.UnfetchedComment != 0 {
		t.Errorf("UnfetchedComment = %d, want 0", iss.UnfetchedComment)
	}
}

// The query must actually ask for the count. Without totalCount in the document
// the field decodes as zero and every long thread looks complete.
func TestIssueQueryAsksForTheCommentTotal(t *testing.T) {
	if !strings.Contains(issueQuery, "totalCount") {
		t.Error("the GraphQL query does not request comments.totalCount; comments past the fetch window would be dropped silently")
	}
}
