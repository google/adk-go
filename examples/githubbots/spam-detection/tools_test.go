...
//
// The counter is atomic because the httptest server calls the handler on its
// own goroutine. Every current caller issues requests one at a time, so the
// happens-before edge comes from the round trip rather than from any
// synchronization here -- which is exactly the kind of accident that stops
// holding the moment a test reuses this helper concurrently.
func countingHandler(calls *atomic.Int32, status int, body string) http.HandlerFunc {
...
func flagHandler(calls *atomic.Int32) http.HandlerFunc {
...
func TestFlagAsSpamRejectsWrongIssueWithoutHTTP(t *testing.T) {
	...
	if calls.Load() != 0 {
		t.Errorf("made %d HTTP calls, want 0", calls.Load())
	}
}
...
func TestFlagAsSpamAuthorizedSucceeds(t *testing.T) {
	...
	if calls.Load() != 2 {
		t.Errorf("made %d HTTP calls, want 2 (label + comment)", calls.Load())
	}
}
...
func TestFlagAsSpamDryRunSucceedsWithoutHTTP(t *testing.T) {
	...
	if calls.Load() != 0 {
		t.Errorf("dry-run made %d HTTP calls, want 0", calls.Load())
	}
}
...
func TestFlagAsSpamSecondCallIsNoOp(t *testing.T) {
	...
	if calls.Load() != 2 {
		t.Errorf("made %d HTTP calls, want 2 (second flag must not call the API)", calls.Load())
	}
}
...
func TestReviewIssueScopesTheSession(t *testing.T) {
	...
	reviewIssue(context.Background(), cfg, 42,
		func(ictx context.Context) {
			...
			} else if remaining := time.Until(dl); remaining > cfg.IssueTimeout || remaining < cfg.IssueTimeout*9/10 {
				// Bounded on BOTH sides. A one-sided check let the deadline be
				// derived from any smaller value -- IssueTimeout/10, say --
				// without failing.
				problems = append(problems, fmt.Sprintf("deadline is %v away, want about %v from IssueTimeout", remaining.Round(time.Second), cfg.IssueTimeout))
			}
		})
	if !ran {
	...
	// The per-issue context must DERIVE from the caller's, not be rooted at
	// Background: a review detached from the run budget would keep going for a
	// further IssueTimeout past it, and be killed mid-write by the workflow's
	// own timeout instead.
	dead, cancel := context.WithCancel(context.Background())
	cancel()
	inherited := false
	reviewIssue(dead, cfg, 42, func(ictx context.Context) {
		inherited = ictx.Err() != nil
	})
	if !inherited {
		t.Error("the per-issue context does not inherit the caller's cancellation")
	}
}
