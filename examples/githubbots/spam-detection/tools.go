...
func (c *GitHubClient) flagAsSpam(ctx context.Context, number int, reason string) (actionResult, error) {
	...
	// post a duplicate alert comment. The claim and the earlier attempt's
	// outcome come from one critical section, so a call that loses the claim
	// cannot report success for a write that is still in flight.
	won, previous := c.markFlagged(number)
	if !won {
		switch previous {
		case flagFailed:
			// Never report success for a write that failed: the model would
			// record an alert that was never posted.
			return errResult("flagging issue #%d already failed this run; it will be retried on the next run", number), nil
		case flagInFlight:
			// Another call holds the claim and its outcome is not known yet.
			return errResult("flagging issue #%d is already in progress this run", number), nil
		default:
