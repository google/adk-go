...
func TestRenderPromptStripsBracesFromConfig(t *testing.T) {
	// A brace arriving via ANY config value must not leak into the instruction,
	// where llmagent would treat it as a missing state key and fail every run.
	// Each field is covered separately: covering only the label left the owner
	// and repo substitutions unpinned.
	for _, tc := range []struct {
		name string
		cfg  *Config
	}{
		{"label", &Config{Owner: "google", Repo: "adk-go", SpamLabel: "spam{bot}"}},
		{"owner", &Config{Owner: "goo{gle}", Repo: "adk-go", SpamLabel: "spam"}},
		{"repo", &Config{Owner: "google", Repo: "adk{go}", SpamLabel: "spam"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if out := renderPrompt(tc.cfg); strings.ContainsAny(out, "{}") {
				t.Errorf("renderPrompt() left stray brace(s):\n%s", out)
			}
		})
	}
}
