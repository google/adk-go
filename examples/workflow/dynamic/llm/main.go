...
func main() {
	...
	myWorkflow := workflow.NewDynamicNode[string, string]("greeter_workflow",
		func(ctx context.Context, nc workflow.NodeContext, in string, _ func(*session.Event) error) (string, error) {
			return workflow.RunNode[string](ctx, nc, greeterNode, in)
		},
		workflow.NodeConfig{},
	)
