...
func Test_inMemoryService_StateDeletionSemantics(t *testing.T) {
	...
	t.Run("app_state_delete_via_patch", func(t *testing.T) {
		s := emptyService(t)
		// Create session with initial app state
		_, _ = s.Create(ctx, &CreateRequest{AppName: appName, UserID: "u1", SessionID: "s1", State: map[string]any{"app:k1": "v1", "app:k2": "v2"}})

		// Delete app:k1 via PatchState with nil value
		_, err := s.PatchState(ctx, &PatchStateRequest{
			AppName:    appName,
			UserID:     "u1",
			SessionID:  "s1",
			StateDelta: map[string]any{"app:k1": nil},
		})
		if err != nil {
		...
		// app:k1 should be deleted from shared app state, app:k2 should remain
		gotState := maps.Collect(s2.Session.State().All())
		if _, exists := gotState["app:k1"]; exists {
			t.Errorf("app:k1 should have been deleted from shared app state but still exists in new session")
		}
		if gotState["app:k2"] != "v2" {
			t.Errorf("app:k2 should be 'v2', got %v", gotState["app:k2"])
		}
	...
	t.Run("user_state_delete_via_patch", func(t *testing.T) {
		s := emptyService(t)
		// Create session with initial user state
		_, _ = s.Create(ctx, &CreateRequest{AppName: appName, UserID: "u1", SessionID: "s1", State: map[string]any{"user:k1": "v1", "user:k2": "v2"}})

		// Delete user:k1 via PatchState with nil value
		_, err := s.PatchState(ctx, &PatchStateRequest{
			AppName:    appName,
			UserID:     "u1",
			SessionID:  "s1",
			StateDelta: map[string]any{"user:k1": nil},
		})
		if err != nil {
			t.Fatalf("PatchState failed: %v", err)
		}
