...
// buildRouter threads the web launcher's configured -max_request_body_size
// limit into the launcher.Config it hands to sublaunchers, so the ADK REST API
// sublauncher can apply the same limit as the base router instead of the
// default.
func TestBuildRouterPropagatesMaxPayloadSizeToSublaunchers(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "caller-set value is preserved", web: 0, caller: int64(1 << 20), want: int64(1 << 20)},
		{name: "flag wins over caller value", web: int64(20 << 20), caller: int64(1 << 20), want: int64(20 << 20)},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := w.buildRouter(&launcher.Config{MaxPayloadSize: tc.caller}); err != nil {
				t.Fatalf("buildRouter() error = %v", err)
			}
...
// TestBuildRouterBaseRouterHonorsEmbedderPayloadLimit verifies that a
// MaxPayloadSize set by an embedder in code, with the -max_request_body_size
// flag left at its zero default, is enforced by the base router itself and not
// only threaded to sublaunchers. buildRouter could regress to feeding the base
// router the raw flag value; an embedder-set limit would then silently stop
// applying to routes mounted on the base router while the sublauncher test
// above stayed green.
func TestBuildRouterBaseRouterHonorsEmbedderPayloadLimit(t *testing.T) {
	const embedderLimit = int64(1 << 20)

	sub := fakeTriggerSublauncher{}
	w := &webLauncher{
		flags:              flag.NewFlagSet("web", flag.ContinueOnError),
		config:             &webConfig{maxPayloadSize: 0},
		sublaunchers:       []Sublauncher{sub},
		activeSublaunchers: map[string]Sublauncher{sub.Keyword(): sub},
	}

	router, err := w.buildRouter(&launcher.Config{MaxPayloadSize: embedderLimit})
	if err != nil {
		t.Fatalf("buildRouter() error = %v", err)
	}
	...
	// A body over the embedder's limit but well under the 10 MiB adkrest
	// default is rejected, so this fails if the base router is wired to the
