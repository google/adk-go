...
func assertAssetsResolve(t *testing.T, router http.Handler, docURL, body string) {
	...
	// zero assets checked would pass every assertion above vacuously. The
	// bound is well under what the bundled UI actually references, so it fires
	// on a broken pattern rather than on the next asset the bundle drops.
	const minLocalAssets = 5
	if local < minLocalAssets {
		t.Errorf("%s: found only %d local asset references in the served document, want at least %d", docURL, local, minLocalAssets)
	}
}
