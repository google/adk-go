...
// debugAPIFlagUsage is a constant because cobra does not wrap flag help, so a
// long literal here runs off the terminal and into the next flag's text.
const debugAPIFlagUsage = "Enable the Debug API - requires '--api'. " +
...
func init() {
	...
	cloudrunCmd.PersistentFlags().BoolVar(&flags.cloudRun.debugAPI, "debug_api", false, debugAPIFlagUsage)
