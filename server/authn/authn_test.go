...
// TestMiddlewareForbiddenWinsWhenErrorWrapsBoth pins the arm order in
// writeAuthError. Only an error wrapping both sentinels depends on it: it is the
// authenticated-but-refused case, so the more specific 403 must win over the
// 401. An error wrapping just one reaches its own arm regardless of order.
func TestMiddlewareForbiddenWinsWhenErrorWrapsBoth(t *testing.T) {
	auth := NewCustom(func(r *http.Request) (*Caller, error) {
		return nil, fmt.Errorf("refused: %w; %w", ErrForbidden, ErrUnauthenticated)
	})
