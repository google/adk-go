...
func (s stubSummarizer) SummarizeEvents(_ context.Context, events []*session.Event) (compaction.SummarizeResult, error) {
	return compaction.SummarizeResult{Content: genai.NewContentFromText(s.text, "model")}, nil
}
...
// runner.New. It exercises the whole chain: ServerConfig.Compaction
...
func TestNewServerRejectsInvalidCompactionConfig(t *testing.T) {
	...
	_, err = adkrest.NewServer(adkrest.ServerConfig{
		...
		Compaction: &compaction.Config{OverlapSize: 2},
	})
	if err == nil {
		t.Fatal("NewServer() accepted an invalid Compaction, want it refused at startup")
	}
...
func TestRESTCompaction_StartupRejectsAConfigItCannotServe(t *testing.T) {
	...
	_, err = adkrest.NewServer(adkrest.ServerConfig{
		...
		Compaction: &compaction.Config{CompactionInterval: 2},
	})
	if err == nil {
