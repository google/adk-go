...
func convertRecords(records []*spanRecord) []DebugSpan {
	...
	records = filterUnrenderable(records)
...
// filterUnrenderable drops spans the ADK web UI's schema rejects.
//
// The UI validates the whole span array in one pass and discards all of it if
// any span fails, so one unrenderable span empties the Traces panel instead of
// hiding its own row. It requires gcp.vertex.agent.event_id on a
// generate_content span, and that attribute is only recorded once the model
// returns a response: TraceGenerateContentResult returns early when the call
// errored, so a failed turn produces a generate_content span without it.
//
// Losing that one span is a poor trade, but it is the better half of it. The
// alternative is that any failed model call — a rejected key, a quota error, a
// 503 — blanks the Traces panel completely, exactly when a developer needs it.
func filterUnrenderable(records []*spanRecord) []*spanRecord {
	return slices.DeleteFunc(records, func(s *spanRecord) bool {
		if s.Name == "" || !strings.HasPrefix(s.Name, generateContentSpanPrefix) {
			return false
		}
		_, ok := s.Attributes[eventIDAttribute]
		return !ok
	})
}
