...
func NewRemoteAgentEvent(ctx context.Context, invCleanCtx agent.InvocationContext) *session.Event {
	...
	event.Author = invCleanCtx.Agent().Name()
	event.Branch = invCleanCtx.Branch()
	return event
}
...
func ToSessionEvent(ctx context.Context, invCleanCtx agent.InvocationContext, event a2a.Event) (*session.Event, error) {
	return ToSessionEventWithParts(ctx, invCleanCtx, event, nil)
}

// ToSessionEventWithParts converts the provided a2a event to session event with custom part converter.
func ToSessionEventWithParts(ctx context.Context, invCleanCtx agent.InvocationContext, event a2a.Event, partConverter A2APartConverter) (*session.Event, error) {
	...
	switch v := event.(type) {
	case *a2a.Task:
		return taskToEvent(ctx, invCleanCtx, v, partConverter)

	case *a2a.Message:
		return messageToEvent(ctx, invCleanCtx, v, partConverter)
	...
	case *a2a.TaskArtifactUpdateEvent:
		...
		event, err := artifactUpdateEventToEvent(ctx, invCleanCtx, v, partConverter)
		if err != nil {
	...
	case *a2a.TaskStatusUpdateEvent:
		if v.Status.State.Terminal() || v.Status.State == a2a.TaskStateInputRequired {
			return finalTaskStatusUpdateToEvent(ctx, invCleanCtx, v, partConverter)
		}
		if v.Status.Message == nil {
			return nil, nil
		}
		event, err := messageToEvent(ctx, invCleanCtx, v.Status.Message, partConverter)
		if err != nil {
...
func messageToEvent(ctx context.Context, invCleanCtx agent.InvocationContext, msg *a2a.Message, partConverter A2APartConverter) (*session.Event, error) {
	if invCleanCtx == nil {
	...
	event := NewRemoteAgentEvent(ctx, invCleanCtx)
	if len(parts) > 0 {
...
func artifactUpdateEventToEvent(ctx context.Context, invCleanCtx agent.InvocationContext, update *a2a.TaskArtifactUpdateEvent, partConverter A2APartConverter) (*session.Event, error) {
	if invCleanCtx == nil {
		return nil, fmt.Errorf("InvocationContext not provided")
	}
	if update == nil {
	...
	allParts, err := convertParts(ctx, invCleanCtx, update, update.Artifact.Parts, partConverter)
	if err != nil {
...
func taskToEvent(ctx context.Context, invCleanCtx agent.InvocationContext, task *a2a.Task, partConverter A2APartConverter) (*session.Event, error) {
	if invCleanCtx == nil {
		return nil, fmt.Errorf("InvocationContext not provided")
	}
	...
	for _, artifact := range task.Artifacts {
		allParts, err := convertParts(ctx, invCleanCtx, task, artifact.Parts, partConverter)
		if err != nil {
			return nil, fmt.Errorf("failed to convert artifact parts: %w", err)
		}
	...
	convertedStatusMsg, err := convertStatusMessage(ctx, invCleanCtx, task, task.Status, partConverter)
	if err != nil {
		return nil, err
	}
...
func finalTaskStatusUpdateToEvent(ctx context.Context, invCleanCtx agent.InvocationContext, update *a2a.TaskStatusUpdateEvent, partConverter A2APartConverter) (*session.Event, error) {
	if update == nil {
		return nil, nil
	}
	...
	convertedStatusMsg, err := convertStatusMessage(ctx, invCleanCtx, update, update.Status, partConverter)
	if err != nil {
		return nil, err
	}
...
func convertParts(ctx context.Context, invCleanCtx agent.InvocationContext, event a2a.Event, parts []*a2a.Part, partConverter A2APartConverter) ([]*genai.Part, error) {
...
func convertStatusMessage(ctx context.Context, invCleanCtx agent.InvocationContext, event a2a.Event, status a2a.TaskStatus, partConverter A2APartConverter) (*convertedStatusMessage, error) {
	...
	convertedParts, err := convertParts(ctx, invCleanCtx, event, parts, partConverter)
	if err != nil {
