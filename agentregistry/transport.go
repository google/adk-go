...
// Transport performs registry API GET requests. Implementations own base-URL
// selection, authentication, and routing. The default implementation talks to
// agentregistry.googleapis.com over REST; alternative implementations (for
// example an internal Borg transport) can be supplied to satisfy the same
// [Client] surface.
...
// restTransport is the default third-party (public) transport. It issues
// authenticated GETs to the Agent Registry REST endpoint.
...
func (t *restTransport) Get(ctx context.Context, resourcePath string, params url.Values, v any) error {
	var fullURL string
	if strings.HasPrefix(resourcePath, "projects/") {
		fullURL = t.baseURL + "/" + resourcePath
	} else {
		fullURL = t.baseURL + "/" + t.basePath + "/" + resourcePath
	}
