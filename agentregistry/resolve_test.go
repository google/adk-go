...
func TestConnectionURI(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantVersion:  "0.3.0",
			wantBinding:  bindingJSONRPC,
			wantOK:       true,
		},
		{
			name:         "binding filter picks matching interface",
			protocols:    agentProtocols,
			protocolBind: bindingHTTPJSON,
			wantURL:      "https://a.example/http",
			wantVersion:  "0.3.0",
			wantBinding:  bindingHTTPJSON,
			wantOK:       true,
		},
		{
			name:         "type and binding filter",
			protocols:    agentProtocols,
			protocolType: "A2A_AGENT",
			protocolBind: bindingHTTPJSON,
			wantURL:      "https://a.example/http",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			gotURL, gotVer, gotBind, gotOK := connectionURI(tc.protocols, tc.ifaces, tc.protocolType, tc.protocolBind)
