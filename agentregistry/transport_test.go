...
// newTestTransport returns a restTransport pointing at srv with a fixed parent
// path and quota project.
func newTestTransport(srv *httptest.Server) *restTransport {
...
func newTestClient(srv *httptest.Server) *Client {
	return &Client{transport: newTestTransport(srv), httpClient: srv.Client()}
}
...
func TestRestTransport_Get_RelativePathAndHeaders(t *testing.T) {
	...
	if err := newTestTransport(srv).Get(context.Background(), "agents", params, &page); err != nil {
...
func TestRestTransport_Get_AbsoluteResourcePath(t *testing.T) {
	var gotPath string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
	...
	if err := newTestTransport(srv).Get(context.Background(), name, nil, &got); err != nil {
		t.Fatalf("Get() error = %v", err)
	}
...
func TestRestTransport_Get_APIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	err := newTestTransport(srv).Get(context.Background(), "agents/missing", nil, &Agent{})
	var apiErr *APIError
...
func TestRestTransport_Get_DecodeError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{not valid json`))
	}))
	defer srv.Close()

	err := newTestTransport(srv).Get(context.Background(), "agents", nil, &AgentsPage{})
	if err == nil {
...
func TestRestTransport_Get_NilOutSkipsDecode(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`not json but ignored`))
	}))
	defer srv.Close()

	if err := newTestTransport(srv).Get(context.Background(), "agents", nil, nil); err != nil {
		t.Errorf("Get() with nil out error = %v, want nil", err)
	}
}

func TestRestTransport_Get_NoUserProjectHeaderWhenEmpty(t *testing.T) {
	var hadHeader bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	rt := &restTransport{httpClient: srv.Client(), baseURL: srv.URL, basePath: "projects/p/locations/l"}
